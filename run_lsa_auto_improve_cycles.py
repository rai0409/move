#!/usr/bin/env python3
"""Run multi-cycle mechanical auto-improvement for TF-IDF + TruncatedSVD(LSA)."""
...
from validate_lsa_candidate_actions import score
...
def selected_approvals(cycle_dir: Path, max_approved: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    actions_path = cycle_dir / "candidates" / "candidate_actions.csv"
    validation_path = cycle_dir / "validation" / "candidate_validation.csv"
    ...
    merged = validation.merge(actions[["candidate_id", "priority_score", "evidence_count"]], on="candidate_id", how="left")
    ...
    return actions[actions["candidate_id"].astype(str).isin(selected_ids)].copy(), validation[validation["candidate_id"].astype(str).isin(selected_ids)].copy()
...
def apply_selected_to_temp(config_dir: Path, actions: pd.DataFrame, validation: pd.DataFrame, temp_config: Path, cycle_dir: Path) -> None:
...
def run_cycles(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for cycle in range(1, args.cycles + 1):
        ...
        actions, validation = selected_approvals(before_run, args.max_approved_per_cycle)
        if actions.empty or validation.empty:
            cycle_rows.append(
                {
                    "cycle": cycle,
        ...
        cycle_rows.append(
            {
                ...
                "reason": "accepted score improvement" if decision == "accepted" else "score improvement below threshold or recall_at_5 dropped",
            }
        )
    ...
    (final_dir / "final_metrics.json").write_text(json.dumps(final_metrics, ensure_ascii=False, indent=2), encoding="utf-8")
